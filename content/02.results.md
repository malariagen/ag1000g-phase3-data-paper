...
The Anopheles gambiae species complex is a crypic group of sibling species, with no single locus offering unambiguous resolution of species.
...
Whole genome SNP calls from 8 arabiensis and XXX gambiae or coluzzii individuals were available.

Alleles were mapped onto the same alternate alleles, the frequencies of which were computed across both groups.

? What about multiallelics? 

? What about missing sites?

400,000 SNPs were identified as potentially informative where the difference in allele frequency exceeded 0.9. 

Given the small number of A. arabiensis samples, and the limited geographical range- it is likely the majority of identified AIMs are not truly informative.

Called genotypes for each individual in the dataset were cross referenced against ancestry informative markers generating a score between 0 (gambiae-like) and 1 (arabiensis-like).

The distribution of markers 
