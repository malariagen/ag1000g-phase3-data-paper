...
By increasing the cutoff for these proportions required to label as PASS, we were able to compute the area under the receiver operating curve (AUROC) for each parameter set.
...
The resulting model was a decision tree of depth 8, with a maximum of 50 terminal nodes, and leaves were assigned to PASS where > 0.533 of training data in that leaf were PASS.

All sites in the genome were assigned to PASS or FAIL given the model inputs.


The 5 remaining cross pedigrees were used to perform a final evaluation of the approach.

The above definitions of PASS sites were retained, but over pedigrees independently, providing 5 distinct evaluation sets.
...
The application of the site filters mask define the accessible fraction of the genome at 70%, and reduce the mendelian error rate by a median factor of 10x on the autosomes.
...
Directly comparing the numbers to the phase 2 site filters, we observe similar levels of mendelian error, however the updated site filters have a substantially higher sensitivity.


- Table A: Mendel errors per cross per chromosome. 
row indexes: chromosome and raw/filtered
column indexes: crosses + % accessible.

Means 10 rows, and 6 columns.
